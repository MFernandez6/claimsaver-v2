...
export const generateClaimPDF = async (claim: ClaimData) => {
  ...
  const addHeader = (y: number) => {
    ...
    pdf.setFontSize(10);
    pdf.text(
      "Personal reference copy — use your insurer’s official form to file",
      pageWidth / 2,
      y + 8,
      {
        align: "center",
    ...
    pdf.setFontSize(8);
    pdf.text(
      `Reference: ${claim.claimNumber || "N/A"} | Saved: ${formatDate(claim.submittedAt || "")}`,
      pageWidth / 2,
      y + 14,
      { align: "center" }
    );
  ...
  const addFooter = () => {
    ...
    pdf.setTextColor(100, 100, 100);
    pdf.text(
      "Personal worksheet for organizing your Florida no-fault information.",
      pageWidth / 2,
      footerY,
      { align: "center" }
    );
    pdf.text(
      "Not a substitute for your insurer’s official application or legal advice.",
      pageWidth / 2,
      footerY + 3,
      { align: "center" }
    );
